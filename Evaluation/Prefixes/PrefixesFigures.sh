...
if [ $# -ne 2 ]; then
    echo "Usage: ./PrefixesFigures.sh [ASes] [Dates]"
    exit 1
fi
...
while [ $i -lt $n_ASes ]
do
    python SubnetPrefixDistribution.py ${ASes[$i]} $datesFile
    mkdir -p ./Figures/"$dstFolder"/${ASes[$i]}/
    mv ${ASes[$i]}-* ./Figures/"$dstFolder"/${ASes[$i]}/
    i=`expr $i + 1`
done
