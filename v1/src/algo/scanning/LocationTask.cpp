...
bool LocationTask::forwardProbing(IPTableEntry *target, unsigned char initTTL)
{
    ...
    target->initRoute();
    RouteHop *newRoute = target->getRoute();
    unsigned short newRouteLength = target->getRouteLength();
    if(newRouteLength > 0 && newRoute != NULL)
    {
...
{
    unsigned short routeLength = target->getRouteLength();
    RouteHop *route = target->getRoute();
    if(route == NULL) // Occurs with interfaces that are one hop away from vantage point
        return;
