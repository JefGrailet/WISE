...
bool IPTableEntry::compareLattestFingerprints(IPTableEntry *ip1, IPTableEntry *ip2)
{
    ...
    if(ah1 != NULL && ah2 != NULL && ah1->isLattestStage() && ah2->isLattestStage())
...
bool IPTableEntry::anonymousEndOfRoute()
{
    if(routeLength == 0 || route == NULL)
        return false;
    return (route[routeLength - 1].ip == InetAddress(0));
}

bool IPTableEntry::setTrail()
{
    // Deletes the previous trail, if any
    if(trail != NULL)
        delete trail;
