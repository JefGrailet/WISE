...
{
    interfaces.push_back(new AliasedInterface(firstInterface, AliasedInterface::FIRST_IP));
}
...
{
    AliasedInterface *newInterface = new AliasedInterface(IP, aliasMethod);
...
void Alias::addInterfaces(list<IPTableEntry*> *IPs, unsigned short aliasMethod)
{
    ...
    while(IPs->size() > 0)
    {
        interfaces.push_back(new AliasedInterface(IPs->front(), aliasMethod));
        IPs->pop_front();
    }
    interfaces.sort(AliasedInterface::smaller);
}
