...
PeerScanner::PeerScanner(Environment *env)
{
    this->env = env;
    ...
    for(list<Subnet*>::iterator i = subnets->begin(); i != subnets->end(); ++i)
    {
         ...
         * subnet is next to the vantage point (i.e., this computer, currently running WISE), 
         * therefore there's no need to look for peers.
         */
