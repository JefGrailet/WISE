...
unsigned short AliasResolver::Ally(IPTableEntry *ip1, IPTableEntry *ip2, unsigned short maxDiff)
{
    ...
    if(ah1 != NULL && ah2 != NULL)
    {
        ...
        unsigned long *probeTokens1 = ah1->getProbeTokens();
        unsigned long *probeTokens2 = ah2->getProbeTokens();
        
        unsigned short *IPIDs1 = ah1->getIPIdentifiers();
        unsigned short *IPIDs2 = ah2->getIPIdentifiers();
...
bool AliasResolver::velocityOverlap(IPTableEntry *ip1, IPTableEntry *ip2)
{
    ...
    if(low1Bis <= up2Bis && up1Bis >= low2Bis)
    {
        ...
        AliasHints *ah_1, *ah_2;
        unsigned long *tokens1 = ah1->getProbeTokens();
        unsigned long *tokens2 = ah2->getProbeTokens();
        ...
        else
        {
            ah_1 = ah2;
            ah_2 = ah1;
            diffTokens = tokens1[0] - tokens2[nbIPIDs - 1];
            
            tokens1 = ah2->getProbeTokens();
            tokens2 = ah1->getProbeTokens();
        }
        
        // We compute a new speed, as increase per tokens, from the first IP.
        unsigned short *IDs1 = ah_1->getIPIdentifiers();
        unsigned short *IDs2 = ah_2->getIPIdentifiers();
...
void AliasResolver::discover(list<IPTableEntry*> IPs, list<Alias*> *results, bool strict)
{
    ...
    for(list<IPTableEntry*>::iterator i = IPs.begin(); i != IPs.end(); ++i)
    {
        ...
        if(curHints != NULL && curHints->isLattestStage())
        ...
        else if(!strict && refHints->getIPIDCounterType() == AliasHints::NO_IDEA)
        {
            ...
            else
            {
                ...
                for(list<IPTableEntry*>::iterator it = similar.begin(); it != similar.end(); ++it)
                {
                    ...
                    if(subCurHints != NULL && !subCurHints->getHostName().empty())
                    {
