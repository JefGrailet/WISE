...
AliasHintsCollector::AliasHintsCollector(Environment *env)
{
    this->env = env;
    tokenCounter = 1;
...
void AliasHintsCollector::collect()
{
    ...
    for(list<IPTableEntry*>::iterator it = backUp1.begin(); it != backUp1.end(); ++it)
    {
        ...
        if(res != IPIDTuples.end())
        {
            ...
            if(curHints == NULL) // Just in case
                continue;
            
            curHints->prepareIPIDData();
            unsigned long *probeTokens = curHints->getProbeTokens();
            unsigned short *IPIdentifiers = curHints->getIPIdentifiers();
            bool *echoes = curHints->getEchoes();
            unsigned long *delays = curHints->getDelays();
