/*
 * AliasHintsCollector.cpp
 *
 *  Created on: Feb 27, 2015
 *      Author: jefgrailet
 *
 * Implements the class defined in AliasHintsCollector.h (see this file to learn further about the 
 * goals of such a class).
 */

#include "AliasHintsCollector.h"
#include "IPIDUnit.h"
...
AliasHintsCollector::AliasHintsCollector(Environment &e): env(e)
{
    tokenCounter = 1;
...
AliasHintsCollector::~AliasHintsCollector()
{
}

void AliasHintsCollector::collect()
{
    ostream *out = env.getOutputStream();
    
    // Sorts the targets and removes possible duplicata
    IPsToProbe.sort(IPTableEntry::compare);
    IPTableEntry *previous = NULL;
    for(list<IPTableEntry*>::iterator i = IPsToProbe.begin(); i != IPsToProbe.end(); ++i)
    {
        IPTableEntry *current = (*i);
        if(current == previous)
            IPsToProbe.erase(i--);
        previous = current;
    }
    ...
    for(list<IPTableEntry*>::iterator i = IPsToProbe.begin(); i != IPsToProbe.end(); ++i)
    {
        ...
        curIP->addHints(AliasHints(nbIPIDs));
        AliasHints &newHints = curIP->getLattestHints();
    ...
    list<IPTableEntry*> backUp1(IPsToProbe);
    list<IPTableEntry*> backUp2(IPsToProbe);
    list<IPTableEntry*> backUp3(IPsToProbe);
    ...
    // Map to save collected IP-ID tuples before treating and storing them in the IP dictionary
    map<IPTableEntry*, IPIDTuple*> IPIDTuples;
    for(list<IPTableEntry*>::iterator it = IPsToProbe.begin(); it != IPsToProbe.end(); ++it)
    {
        IPIDTuple *newArray = new IPIDTuple[nbIPIDs];
        IPIDTuples.insert(pair<IPTableEntry*, IPIDTuple*>((*it), newArray));
    }
    ...
    list<IPTableEntry*> roundTargets = IPsToProbe;
    for(unsigned short i = 0; i < nbIPIDs; i++)
    {
        ...
        {
            IPTableEntry *IPToProbe = roundTargets.front();
        ...
        for(list<IPIDTuple>::iterator it = tuples.begin(); it != tuples.end(); ++it)
        {
            if(prev.probeToken != 0)
            {
                ...
                unsigned short maxDiff = env.getARAllyMaxConsecutiveDiff();
                unsigned short diff = cur.IPID - prev.IPID;
                if(diff <= maxDiff && cur.probeToken < prev.probeToken)
                {
                    reordered++;
                    ...
                    for(list<IPIDTuple>::reverse_iterator it2 = tuples.rbegin(); it2 != tuples.rend(); ++it2)
                    {
                        ...
                        if(diffBis <= maxDiff && curBis->probeToken > prevBis->probeToken)
                        {
        ...
        while(tuples.size() > 0)
        {
            ...
            map<IPTableEntry*, IPIDTuple*>::iterator res = IPIDTuples.find(curTuple.target);
    ...
    for(list<IPTableEntry*>::iterator it = backUp1.begin(); it != backUp1.end(); ++it)
    {
        IPTableEntry *curIP = (*it);
        map<IPTableEntry*, IPIDTuple*>::iterator res = IPIDTuples.find(curIP);
        if(res != IPIDTuples.end())
        {
            IPIDTuple *tuplesArray = res->second;
            
            // Gets and prepares the hints
            AliasHints &curHints = curIP->getLattestHints();
            if(curHints.isEmpty()) // Just in case
                continue;
            
            curHints.prepareIPIDData();
            vector<unsigned long> &probeTokens = curHints.getProbeTokens();
            vector<unsigned short> &IPIdentifiers = curHints.getIPIdentifiers();
            vector<bool> &echoes = curHints.getEchoes();
            vector<unsigned long> &delays = curHints.getDelays();
            ...
            for(unsigned short i = 0; i < nbIPIDs; i++)
            {
                ...
                probeTokens[i] = curTuple.probeToken;
                IPIdentifiers[i] = curTuple.IPID;
                if(curTuple.echo)
                    echoes[i] = true;
            ...
            if(inferredInitialTTL > 0 && !differentTTLs)
                curHints.setEchoInitialTTL(inferredInitialTTL);
            
            delete[] tuplesArray;
        }
    }
    IPIDTuples.clear(); // Explicity tells we don't need the IPIDTuples map anymore here
    ...
    {
        IPTableEntry *IPToProbe = backUp1.front();
        backUp1.pop_front();
    ...
    {
        IPTableEntry *IPToProbe = backUp2.front();
        backUp2.pop_front();
    ...
    {
        IPTableEntry *IPToProbe = backUp3.front();
        backUp3.pop_front();
